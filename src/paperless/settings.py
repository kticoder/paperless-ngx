...
def __get_optional_int(key: str) -> Optional[int]:
    ...
    """
    return __get_int(key, -1) if key in os.environ else None
...
def __get_optional_path(key: str) -> Optional[Path]:
    ...
    """
    return __get_path(key, "") if key in os.environ else None
...
if AUDIT_LOG_ENABLED := __get_boolean("PAPERLESS_AUDIT_LOG_ENABLED", "NO"):
